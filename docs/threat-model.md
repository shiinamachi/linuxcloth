...
| Installation media | The user-selected Windows ISO and optional local virtio fallback receive size-bounded, network-disabled Bubblewrap-confined 7-Zip probes across ISO9660/UDF and SHA-256 hashing. Only fixed required entries can be probed or extracted. The normal driver ISO comes from a release-pinned immutable URL and is length/hash verified before atomic private-cache promotion. Media are not uploaded. |
