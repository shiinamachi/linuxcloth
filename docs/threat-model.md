...
| Installation media | The user-selected Windows ISO and optional local virtio fallback receive size-bounded, network-disabled Bubblewrap-confined content probes and SHA-256 hashing. The normal driver ISO comes from a release-pinned immutable URL and is length/hash verified before atomic private-cache promotion. Media are not uploaded. |
